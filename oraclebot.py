...
def get_location(city, country):
    ...
    geolocator = Nominatim(user_agent="horacle-astrology-app")  
    ...
    try:
        ...
        if location:
            ...
            print(f"✅ Found: {lat}, {lon} (for {location_query})")
            return lat, lon, "UTC"  # ⏳ Default timezone
        else:
            print(f"❌ Location not found: {location_query}")
            return None, None, None
    except GeocoderTimedOut:
        print("❌ Geopy request timed out.")
        return None, None, None
...
def generate_birth_chart(name, year, month, day, hour, minute, city, country):
    print(f"🔍 Generating chart for: {city}, {country}")  
    ...
    print(f"✅ Using location: {lat}, {lon}, Timezone: {timezone}")

    try:
        # ✅ Pass latitude, longitude, and timezone directly
        subject = AstrologicalSubject(
            ...
            hour=hour,
            minute=minute,
            latitude=lat,    # ✅ Manually setting lat
            longitude=lon,   # ✅ Manually setting lon
            timezone=timezone  # ✅ Manually setting timezone
        )
...
def main():
    ...
    page = st.sidebar.radio(
        "Go to", 
        ["Chat", "Current Astrology", "Charts", "Tokens"]
    )
    ...
    if page == "Chat":
        ...
        if st.button("Generate Birth Chart"):
            chart_svg = generate_birth_chart(name, year, month, day, hour, minute, city, country)
